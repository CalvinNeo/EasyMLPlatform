...
def find_frequent_itemsets(transactions, minimum_support, include_support=False):
    ...
    def find_with_suffix(tree, suffix):
        # for nodes in tree.items():
        #    print "HAHA",str(nodes)
        for item, nodes in tree.items():
            support = sum(n.count for n in nodes)
            if support >= minimum_support and item not in suffix:
                # New winner!
                print "item",item,"suffix",suffix
...
def fp_growth(data,freq,show_support = True):
    itemsets = find_frequent_itemsets(data, freq, show_support)#for itemset, support in iterator
    print itemsets

if __name__ == '__main__':
    fp_growth([[item for item in str(line).strip().split()] for line in open("dat.txt","r").readlines()],2)